...
extern pthread_mutex_t stats_lock ; //= PTHREAD_MUTEX_INITIALIZER;
...
int build_fd_sets(fd_set *read_fds, fd_set *write_fds, fd_set *except_fds)
{
  ...
  FD_SET(listen_sock, read_fds);                                    // To be added file changes
  for (i = 0; i < MAX_CLIENTS; ++i)
    if (connection_list[i].socket != NO_SOCKET)
...
int main(int argc, char **argv)
{
  ...
  */  
  ...
  printf("::Creating 3 Threads for handling.\n");
  struct Graph* graph = create_graph(ONE_FILE);
  pthread_create(&tid, NULL, shortest_path, (void*)graph);
  pthread_create(&tid, NULL, minimum_spanning_tree, (void*)graph);
  pthread_create(&tid, NULL, file_changes_update, (void*)ONE_FILE);
